/* empty css                                          */
import { f as createAstro, g as createComponent, m as maybeRenderHead, i as addAttribute, r as renderTemplate, j as renderComponent } from '../../chunks/astro/server_CE9mvu6d.mjs';
import 'kleur/colors';
import { a as $$Layout } from '../../chunks/Layout_DgNdm22p.mjs';
import { G as GRUPOSPROYECCIONES, C as CINES, P as PELICULAS } from '../../chunks/proyecciones_BcPbmD8H.mjs';
import 'clsx';
/* empty css                                         */
...
const $$ProyeccionesPelicula = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="rounded-2xl border-2 bg-white dark:bg-gray-900 bg-opacity-90 shadow-lg overflow-hidden cine-card"${addAttribute(`--cine-color: var(--color-${cine.color})`, "style")} data-astro-cid-autajkkl> <div class="cine-card__header px-4 py-2.5 flex items-center justify-between gap-2" data-astro-cid-autajkkl> <h3 class="text-white text-sm font-extrabold truncate" data-astro-cid-autajkkl>${cine.nombre}</h3> <a${addAttribute(`/cartelera-${cine.url}`, "href")} class="text-white/90 text-[11px] font-semibold whitespace-nowrap flex items-center gap-1 hover:text-white transition-colors" data-astro-cid-autajkkl>
Ver cartelera
<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" data-astro-cid-autajkkl> <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" data-astro-cid-autajkkl></path> </svg> </a> </div> ${proyecciones.length === 0 ? renderTemplate`<div class="p-4 text-center" data-astro-cid-autajkkl> <p class="text-sm text-gray-600 dark:text-gray-400" data-astro-cid-autajkkl>Sin proyecciones disponibles en este cine.</p> </div>` : renderTemplate`<div class="p-3 grid grid-cols-2 gap-2" data-astro-cid-autajkkl> ${diasProyeccion.map((dia) => {
    ...
    return renderTemplate`<div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-center" data-astro-cid-autajkkl> <p class="cine-card__day text-xs font-extrabold" data-astro-cid-autajkkl>${diaLabel}</p> <p class="text-xs text-gray-700 dark:text-gray-300 mt-0.5 leading-relaxed" data-astro-cid-autajkkl>${horas}</p> </div>`;
  })} </div>`} </div> `;
