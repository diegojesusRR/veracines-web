import { G as GRUPOSPROYECCIONES, C as CINES, a as $$Layout } from '../chunks/Layout_CVDcOek8.mjs';
...
- ${proyeccionesDia.map((proyeccion) => `${proyeccion.hora} ${proyeccion.vose ? " (VOSE)" : ""}`).join(", ")} ${ultimaProyeccion && ` y ${ultimaProyeccion.hora} ${ultimaProyeccion.vose ? " (VOSE)" : ""}`} </li>`;
  })} </ul> </div> ${entradasLink && proyecciones[proyecciones.length - 1].fecha >= /* @__PURE__ */ new Date() && renderTemplate`${renderComponent($$result, "Action", $$Action, { "as": "a", "class": `w-3/4 sm:text-xs my-1`, "aria-label": `Enlace a los tickets de ${cine.nombre} para la pel\xEDcula ${pelicula.nombre}`, "href": entradasLink, "color": cine.color }, { "default": ($$result2) => renderTemplate`
Comprar entradas
` })}`} ${entradasVoseLink && proyecciones[proyecciones.length - 1].fecha >= /* @__PURE__ */ new Date() && renderTemplate`${renderComponent($$result, "Action", $$Action, { "as": "a", "class": `w-3/4 sm:text-xs`, "aria-label": `Enlace a la cartelera de ${cine.nombre} para la pel\xEDcula ${pelicula.nombre} en VOSE`, "href": entradasVoseLink, "color": cine.color }, { "default": ($$result2) => renderTemplate`
...
const $$ProyeccionesCine = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section${addAttribute(`proyecciones-${cine.url}`, "id")}> ${GRUPOSPROYECCIONES.length === 0 ? renderTemplate`<div${addAttribute(`p-2 m-4 align-middle text-${cine.color}`, "class")}> <p>Actualmente no hay proyecciones disponibles.</p> </div>` : GRUPOSPROYECCIONES.filter((x) => x.cine.url === cine.url).map((grupo) => {
    return renderTemplate`${renderComponent($$result, "ProyeccionCard", $$ProyeccionCard, { "cine": cine, "entradas": grupo.entradas, "proyecciones": grupo.proyecciones, "pelicula": grupo.pelicula })}`;
  })} </section>`;
