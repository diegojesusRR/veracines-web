...
import 'kleur/colors';
import { C as CINES, a as $$Layout } from '../chunks/Layout_RJOfwuWe.mjs';
import 'clsx';
...
const $$UbicacionCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="flex flex-col sm:flex-row p-4 m-2 rounded-2xl bg-gradient-to-br from-white via-gray-100 to-gray-200 border-2 border-primary shadow-xl transition-transform md:hover:scale-105"> <div class="flex flex-col justify-center items-center p-6 sm:w-2/3"> <h4${addAttribute(`text-2xl text-${cine.color} text-center font-extrabold underline mb-3 drop-shadow-md`, "class")}> ${cine.nombre} </h4> <p${addAttribute(`text-center text-${cine.color} mb-4 text-base font-medium`, "class")}> ${cine.localizacion.descripcion} </p> ${renderComponent($$result, "Action", $$Action, { "class": "w-full md:w-3/4 text-base md:text-xs bg-primary text-white font-bold py-2 px-4 rounded-lg shadow hover:bg-opacity-90 transition", "color": "white", "as": "a", "aria-disabled": "false", "href": `/cartelera-${cine.url}/`, "aria-label": `Enlace a la cartelera de ${cine.nombre}` }, { "default": ($$result2) => renderTemplate`
🎬 Consultar cartelera
` })} </div> <div class="flex flex-col items-center justify-center"> <div class="border-4 border-primary rounded-xl overflow-hidden shadow-lg m-auto"> <iframe class="align-middle w-full rounded-lg"${addAttribute(`https://www.google.com/maps/embed?pb=${cine.localizacion.ubicacionKey}`, "src")} width="400" height="250" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe> </div> <p${addAttribute(`text-center underline text-${cine.color} text-xs font-bold w-auto p-2 mt-2 bg-white bg-opacity-60 rounded`, "class")}>
...
const $$Ubicaciones = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section> <h3 class="text-xl text-primary text-center font-bold">Puedes encontrarnos en:</h3> ${renderTemplate`${renderComponent($$result, "UbicacionCard", $$UbicacionCard, { "cine": vera })}
        ...
        <p class="text-center text-red-700 font-extrabold text-lg bg-red-100 border border-red-400 rounded px-4 py-2 mt-4 shadow-md">
